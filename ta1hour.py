# -*- coding: utf-8 -*-
"""
Created on Fri Mar 22 12:31:41 2024
...
def append_historical_data():
    try:
        ...
        dax_data = tv.get_hist("DE30EUR", 'OANDA', interval=Interval.in_1_hour, n_bars=500)
        ...
        print("Historical data appended successfully 1 hour.",datetime.now())
        #return True
    ...
    while True:
        try:
            with excel_lock:
                ...
                current_hour = pd.Timestamp.now().replace(minute=0, second=0, microsecond=0)
                current_row_index = len(df.open) - 1
                
                current_data = get_indicators()
                new_hour = pd.Timestamp.now().replace(minute=0, second=0, microsecond=0)
                
                if new_hour != current_hour:
                    df.loc[current_row_index - 1, "close"] = df.loc[current_row_index, "open"]
                    current_row_index += 1
                    df.loc[current_row_index, 'timestamp'] = new_hour
